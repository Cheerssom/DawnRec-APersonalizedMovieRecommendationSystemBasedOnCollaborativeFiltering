    ...
    try {
        ...
        const content = `
            <div class="row">
                <div class="col-md-4">
                    ${movie.poster_url ? 
                        `<img src="${movie.poster_url}" class="img-fluid rounded mb-3 shadow-sm w-100" alt="${movie.title}" referrerpolicy="no-referrer" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                         <div class="bg-light rounded mb-3 shadow-sm w-100 align-items-center justify-content-center" style="height: 300px; display: none;"><i class="fas fa-film fa-3x text-muted"></i></div>` : 
                        `<div class="bg-light rounded mb-3 shadow-sm w-100 d-flex align-items-center justify-content-center" style="height: 300px;"><i class="fas fa-film fa-3x text-muted"></i></div>`
                    }
                    <div class="card">
                        <div class="card-body text-center">
                            <h3>${movie.avg_score || 0}</h3>
                            <div class="text-warning">
                                ${'★'.repeat(Math.max(0, Math.min(5, Math.floor(movie.avg_score || 0))))}
                                ${'☆'.repeat(Math.max(0, Math.min(5, 5 - Math.floor(movie.avg_score || 0))))}
                            </div>
        ...
        // 显示模态框
        let modal = bootstrap.Modal.getInstance(document.getElementById('movieDetailModal'));
        if (!modal) {
            modal = new bootstrap.Modal(document.getElementById('movieDetailModal'));
        }
        modal.show();
